...
from socaity.core.jobs.threaded.internal_job_manager import InternalJobManager
...
class ServiceClientAPI:
    ...
    def __init__(self, service_name: str, service_url: str):
        self.service_client = ServiceClient() # #registry.get_service(service_name_or_service_client)
        self.internal_job_manager = InternalJobManager()

    def request(self, endpoint_route: str, *args, **kwargs):
        return self.service_client(endpoint_route=endpoint_route, call_async=True, *args, **kwargs)

    def request_sync(self, endpoint_route: str, *args, **kwargs):
        endpoint_request = self.request(endpoint_route=endpoint_route, *args, **kwargs)
        return endpoint_request.wait_until_finished()

    def job(self):
        def decorator(func):
            def wrapper(*args, **kwargs):
                self.internal_job_manager
                # spawn internal job
