...
class aura_flow(FastSDK):
    """
    Generated client for aura_flow
    """
    def __init__(self, api_key: str = None):
        super().__init__(service_name_or_id="3f9c902b-86ce-47d9-b48b-8bb229b6bfed", api_key=api_key)
    
    def predict(self, cfg: float = 3.5, shift: float = 1.73, steps: int = 25, width: int = 1024, height: int = 1024, prompt: str = '', sampler: str = 'uni_pc', scheduler: str = 'normal', output_format: str = 'webp', output_quality: int = 80, negative_prompt: str = '', number_of_images: int = 1, seed: Optional[int] = None, **kwargs):
        """
        ...
        """
        return self.submit_job("/predict", cfg=cfg, shift=shift, steps=steps, width=width, height=height, prompt=prompt, sampler=sampler, scheduler=scheduler, output_format=output_format, output_quality=output_quality, negative_prompt=negative_prompt, number_of_images=number_of_images, seed=seed, **kwargs)
