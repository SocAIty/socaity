...
class arcodeltriunfosjv1(FastClient):
    """
    Generated client based on mofleck/arcodeltriunfosjv1 format
    """
    def __init__(self, api_key: str = None):
        super().__init__(service_name_or_id="00c5faf8-85d3-4441-a71c-541729fddfe2", api_key=api_key)
    
    def predictions(self, prompt: str, seed: int = 42, model: Literal["dev", "schnell"] = 'dev', go_fast: bool = False, lora_scale: float = 1.0, megapixels: Literal["1", "0.25"] = '1', num_outputs: int = 1, aspect_ratio: Literal["1:1", "16:9", "21:9", "3:2", "2:3", "4:5", "5:4", "3:4", "4:3", "9:16", "9:21", "custom"] = '1:1', output_format: Literal["webp", "jpg", "png"] = 'webp', guidance_scale: float = 3.0, output_quality: int = 80, prompt_strength: float = 0.8, extra_lora_scale: float = 1.0, num_inference_steps: int = 28, disable_safety_checker: bool = False, mask: Optional[MediaFile] = None, image: Optional[MediaFile] = None, width: Optional[int] = None, height: Optional[int] = None, extra_lora: Optional[str] = None, replicate_weights: Optional[str] = None, **kwargs) -> APISeex:
        """
        ...
        """
        return self.submit_job("/predictions", prompt=prompt, seed=seed, model=model, go_fast=go_fast, lora_scale=lora_scale, megapixels=megapixels, num_outputs=num_outputs, aspect_ratio=aspect_ratio, output_format=output_format, guidance_scale=guidance_scale, output_quality=output_quality, prompt_strength=prompt_strength, extra_lora_scale=extra_lora_scale, num_inference_steps=num_inference_steps, disable_safety_checker=disable_safety_checker, mask=mask, image=image, width=width, height=height, extra_lora=extra_lora, replicate_weights=replicate_weights, **kwargs)
