...
class controlnet_normal(FastSDK):
    """
    Generated client for controlnet_normal
    """
    def __init__(self, api_key: str = None):
        super().__init__(service_name_or_id="cd1665cc-b085-458c-acf2-a0ea6e31baa4", api_key=api_key)
    ...
    def predict(self, image: Union[MediaFile, str, bytes], prompt: str, eta: float = 0.0, scale: float = 9.0, a_prompt: str = 'best quality, extremely detailed', n_prompt: str = 'longbody, lowres, bad anatomy, bad hands, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality', ddim_steps: int = 20, num_samples: str = '1', bg_threshold: float = 0.0, image_resolution: str = '512', detect_resolution: int = 512, seed: Optional[int] = None, **kwargs):
        ...
        """
        return self.submit_job("/predict", image=image, prompt=prompt, eta=eta, scale=scale, a_prompt=a_prompt, n_prompt=n_prompt, ddim_steps=ddim_steps, num_samples=num_samples, bg_threshold=bg_threshold, image_resolution=image_resolution, detect_resolution=detect_resolution, seed=seed, **kwargs)
