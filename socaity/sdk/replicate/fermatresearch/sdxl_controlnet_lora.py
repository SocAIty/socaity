...
class sdxl_controlnet_lora(FastSDK):
    """
    Generated client for sdxl_controlnet_lora
    """
    def __init__(self, api_key: str = None):
        super().__init__(service_name_or_id="57c1848e-1b4f-4a1f-8d92-2efc16171e97", api_key=api_key)
    
    def predict(self, prompt: str = 'An astronaut riding a rainbow unicorn', refine: str = 'base_image_refiner', strength: float = 0.8, scheduler: str = 'K_EULER', lora_scale: float = 0.95, num_outputs: int = 1, refine_steps: int = 10, guidance_scale: float = 7.5, apply_watermark: bool = True, condition_scale: float = 1.1, negative_prompt: str = '', num_inference_steps: int = 30, seed: Optional[int] = None, image: Optional[Union[MediaFile, str, bytes]] = None, img2img: Optional[bool] = None, lora_weights: Optional[str] = None, **kwargs):
        """
        ...
        """
        return self.submit_job("/predict", prompt=prompt, refine=refine, strength=strength, scheduler=scheduler, lora_scale=lora_scale, num_outputs=num_outputs, refine_steps=refine_steps, guidance_scale=guidance_scale, apply_watermark=apply_watermark, condition_scale=condition_scale, negative_prompt=negative_prompt, num_inference_steps=num_inference_steps, seed=seed, image=image, img2img=img2img, lora_weights=lora_weights, **kwargs)
