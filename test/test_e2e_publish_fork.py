...
def run() -> None:
    ...
    with owner:
        ...
        published = client.publish_workflow(wf_id)
    ...
    with second:
        fetched = client.get_workflow(wf_id)
        ...
        fork = None
        for _ in range(15):
            mine = client.query_workflows(filters=["visibility:eq:mine"], limit=50)
        ...
        fork_revisions = client.query_workflow_revisions(fork.id)
        env.log("T2B", f"fork revisions: {[(r.id, r.message) for r in fork_revisions]}")
        fork_doc = client.get_workflow(fork.id).document
        ...
        reverted = client.revert_workflow(fork.id, revision_id=base_revision.id)
        assert reverted is not None, "revert failed"
        after = client.get_workflow(fork.id).document
